...
public class DekkerSolverImpl implements DekkerSolver {
    ...
    public List<DekkerResult> solve(Function f, double a, double b, double tol)
            throws DekkerException {

        return solve(f, a, b, tol, DEFAULT_MAX_ITERATIONS);
    }

    public List<DekkerResult> solve(Function f, double a, double b, double tol,
            int maxIterations) throws DekkerException {
        ...
        if (sameSigns(fa_0, b_0)) {
            // TODO Throws exception - This interval is not valid.
        }

        // b = a;
        double cA;// the current value of A
        double cB; // the current value of b - At the end of iterations this
                   // value should be the best guess for the
        // solution
        double oldB;// the old value of b
        double s;// this value is calculated based on secant method
        double m;// this value is calculated based on bisection method
        List<DekkerResult> guesses = new LinkedList<DekkerResult>();

        // Initialization
        // cB must be the best guess for the solution
        ...
        int iter = 1;
        guesses.add(new DekkerResultImpl(cB));// cB is our first guess
        ...
        if (iter > maxIterations) {
            throw new DekkerException(
                    "The best guess was not found in less then "

 ...
 */
public class BusAlgorithmRSolverImpl implements BusAlgorithmRSolver {
    ...
    public List<BusAlgorithmRResult> solve(Function f, double a, double b,
            double tol) throws BusAlgorithmRException {

        return solve(f, a, b, tol, DEFAULT_MAX_ITERATIONS);
    }

    public List<BusAlgorithmRResult> solve(Function f, double a, double b,
            double tol, int n) throws BusAlgorithmRException {

	int signA = signum(f.calculate(a)) < 0 ? -1 : 1;
	int signB = signum(f.calculate(b)) < 0 ? -1 : 1;
	if(signA == signB){
	    throw new BusAlgorithmRException(String.format("ERROR! The ordinates of the abscissas %f and %f have the same sign.", a, b));
	}
        List<BusAlgorithmRResult> guesses = new LinkedList<BusAlgorithmRResult>();
        ...
        for (i++; i <= n; i++) {
            ...
            oldC = c_i;
            oldD = d_i;
            guesses.add(new BusAlgorithmRResultImpl(b_i));
        ...
        if (i > n) {
            throw new BusAlgorithmRException(
     ...
     */
    private int getJ() throws BusAlgorithmRException {
     ...
     * @throws BusAlgorithmRException 
     */
    private double getX_K() throws BusAlgorithmRException {
    ...
    public List<BusAlgorithmRResult> solve(Function funcion, double tolerance) {

        throw new UnsupportedOperationException("This operation is not supported by this method");
    }

    private BusAlgorithmRException generateError(){
	
	return new BusAlgorithmRException("ERROR! Please, contact the developers at <edigley at gmail dot com> and <jaindsonvs at gmail dot com>.\nAlso, don't forget to send the parameters you have used to us.");
    }
